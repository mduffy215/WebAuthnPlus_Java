 ...
 * 
 * Order of precedence within io.trustnexus.struts.actions.createcredential:  22
 */
public class CancelCreateCredentialAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {

  private static final long serialVersionUID = -3489163381348414378L;

  private static Logger logger = LogManager.getLogger(CancelCreateCredentialAction.class);
  ...
  public String execute() {
    ...
    try {
      ...
      /*
       * Test the hashed values to determine the integrity of the message. If the hashed values are equal continue; 
       * else return a message:  PropertyManager.getInstance().getProperty(Constants.MESSAGE_INTEGRITY_COMPROMISED).
       */
        ...
        if (verifyTransactionUuidMessage.equals(PropertyManager.getInstance().getProperty(Constants.TRANSACTION_UUID_VERIFIED))) {
          ...
          if (verifySignatureMessage.trim().equals(PropertyManager.getInstance().getProperty(Constants.SIGNATURE_VERIFIED))) { 
            ...
            if (credential != null) {
              ...
              visitor.setRemoteAddress(credential.getRemoteAddress());
              visitor.setRemoteHost(credential.getRemoteHost());
              visitor.setRefCode(Constants.VISITOR_CREATE_CREDENTIAL);

              VisitorThreatDao.create(visitor);            
              
              CredentialDao.cancelCreate(userUuid, sessionUuid);
              responseString = PropertyManager.getInstance().getProperty(Constants.CREATE_IDENTITY_CREDENTIAL_CANCELED);
