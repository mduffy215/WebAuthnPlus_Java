...
/*
 * This action is called from the WebAuthn+ mobile app when the user requests a transaction UUID.  All 
 ...
 */
public class RetrieveTransactionUuidAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {

  private static final long serialVersionUID = -7271714179120664161L;

  private static Logger logger = LogManager.getLogger(RetrieveTransactionUuidAction.class);
  ...
  public String execute() {
    ...
    try {
      ...
      if (transferDataEncryptedHashedHex.equals(transferDataEncryptedHashedHexTest)) {
        ...
        logger.debug("verifyTransactionUuidMessage: " + verifySignatureMessage);

        if (verifySignatureMessage.trim().equals(PropertyManager.getInstance().getProperty(Constants.SIGNATURE_VERIFIED))) {

          String returnTransactionUuid = CryptoUtilities.generateUuid();
          logger.debug("returnTransactionUuid: " + returnTransactionUuid);

          Transaction transaction = new Transaction();

          transaction.setUpdatedById(Constants.MASTER_USER);
          transaction.setDataSourceTypeValue(Constants.DATA_SOURCE_APPLICATION);
          transaction.setUserUuid(userUuid);
          transaction.setTransactionUuid(returnTransactionUuid);
          transaction.setInactiveFlag(false);

          TransactionDao.create(transaction);
        
          responseString = Constants.TRANSACTION_UUID + "=" + returnTransactionUuid + "&";            
