...
/*
 * This action is called from the WebAuthn+ mobile app when the user enters his/her HEX pin and "Activates" the 
 * application.  All communication between the WebAuthn+ mobile app and the TNX Secure web application is 
 ...
 */
public class MobileApplicationSignOnAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {

  private static final long serialVersionUID = 9084174040917501953L;

  private static Logger logger = LogManager.getLogger(MobileApplicationSignOnAction.class);
  ...
  public String execute() {
    ...
    /*
     * UserSessionTrackingDao is not called because the mobile app sign on process obfuscates the user.
     */
    ...
    try {
       ...
       */
      if (transferDataEncryptedHashedHex.equalsIgnoreCase(transferDataEncryptedHashedHexTest)) {
        ...
        logger.debug("userIdentifier: " + obfuscatedIdentifier);

        SecurityKey securityKey = SecurityKeyDao.retrieveByObfuscatedIdentifier(obfuscatedIdentifier);

        /*
         * If the user exists, process the sign on; else, return a message:  Constants.USER_IDENTIFIER_DOES_NOT_EXIST.
         */
        if (securityKey != null) {

          /*
           * Wrap the user's userSecurityKeyEncrypted with the transfer key and return the result to the user.
           * The userSecurityKey was already encrypted in the WebAuthn+ mobile app when the user was activated, so this 
           * wrapping is a little bit excessive.
           */
          String wrappedUserSecurityKeyEncrypted = CryptoUtilities.wrapUserSecurityKeyEncrypted(transferKey, securityKey.getUserSecurityKeyEncrypted());

          responseString = Constants.MESSAGE + "=" + PropertyManager.getInstance().getProperty(Constants.SIGN_ON_SUCCESSFUL) + "&" 
                         + Constants.USER_SECURITY_KEY_ENCRYPTED + "=" + wrappedUserSecurityKeyEncrypted.trim() + "&";

        } else {
          responseString = PropertyManager.getInstance().getProperty(Constants.OBFUSCATED_IDENTIFIER_DOES_NOT_EXIST);
        }
      } else {
      ...
      responseString = PropertyManager.getInstance().getProperty(Constants.INCORRECT_PIN);
    } catch (Exception e) {
