...
/*
 * This action is called from the WebAuthn+ mobile app in the process of creating a credential.  All  
 * communication between the WebAuthn+ mobile app and the TNX Secure web application is encrypted based on the 
 ...
 */
public class RetrieveTnxCredentialMetaDataAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {

  private static final long serialVersionUID = 1771432310692776205L;

  private static Logger logger = LogManager.getLogger(RetrieveTnxCredentialMetaDataAction.class); 
  ...
  public String execute() {
    ...
    try {
      ...
      /*
       * Use the TNX private key to decrypt the transfer key.
       */
      ...
      if (transferDataEncryptedHashedHex.equals(transferDataEncryptedHashedHexTest)) {
        ...
        if (verifyTransactionUuidMessage.trim().equals(PropertyManager.getInstance().getProperty(Constants.TRANSACTION_UUID_VERIFIED))) {
          ...
          if (verifySignatureMessage.trim().equals(PropertyManager.getInstance().getProperty(Constants.SIGNATURE_VERIFIED))) {
            ...
            if (credential != null && (credential.getVerificationCode() == null || credential.getVerificationCode().length() == 0)) {
            	
              logger.debug("credential: " + credential.toString());
            	
            	String credentialMetaDataNameValuePairs = CredentialProviderDao.retrieveCredentialMetaDataNameValuePairs(credential);
              logger.debug("credentialProviderNameValuePairs: " + credentialMetaDataNameValuePairs);
              
						  responseString = credentialMetaDataNameValuePairs; 
