...
import io.trustnexus.jdbc.VisitorThreatDao;
...
/*
 * This action is called from the WebAuthn+ mobile app when the user touches the "Cancel" option when creating
 * a credential.  All communication between the WebAuthn+ mobile app and the TNX Secure web application is 
 * encrypted based on the Transport Layer Security protocol with no digital certificates being used:  
 ...
 * 
 * Order of precedence within io.trustnexus.struts.actions.signon:  33
 */
public class CancelSignOnAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {

    private static final long serialVersionUID = -3489163381348414378L;

    private static Logger logger = LogManager.getLogger(CancelSignOnAction.class);
    ...
    public String execute() {
        ...
        try {
            ...
            /*
             * The HTTP request contains four packages (key/value pairs). The first package
             * is the publicKeyUuid. This UUID is used to lookup the CREDENTIAL_PROVIDER
             * private key.
             */
            ...
            /*
             * The second HTTP request package contains the transferKeyEncryptedHex which
             * was encrypted with the publicKey.
             */
            ...
            byte[] transferKeyDecryptedBytes = rsaCipher
                    .doFinal(CryptoUtilities.hexStringToByteArray(transferKeyEncryptedHex));
            ...
            /*
             * The third HTTP request package contains the transfer data encrypted with the
             * transferKey.
             */
             ...
             * hashed values are equal continue; else return a message:
             * PropertyManager.getInstance().getProperty(Constants.
            ...
            if (transferDataEncryptedHashedHex.equals(transferDataEncryptedHashedHexTest)) {
                 ...
                 */
                cipher.init(Cipher.DECRYPT_MODE, transferKey, new IvParameterSpec(
                        CryptoUtilities.hexStringToByteArray(transferDataEncryptedHex.substring(0, 32))));

                byte[] transferDataEncryptedBytes = CryptoUtilities
                        .hexStringToByteArray(transferDataEncryptedHex.substring(32));
                byte[] transferDataDecryptedBytes = new byte[cipher.getOutputSize(transferDataEncryptedBytes.length)];

                int transferDataDecryptionPointer = cipher.update(transferDataEncryptedBytes, 0,
                        transferDataEncryptedBytes.length, transferDataDecryptedBytes, 0);
                transferDataDecryptionPointer += cipher.doFinal(transferDataDecryptedBytes,
                        transferDataDecryptionPointer);
                ...
                String transactionUuid = Utilities.parseNameValuePairs(transferDataDecrypted,
                        Constants.TRANSACTION_UUID);
                logger.debug("transactionUuid: " + transactionUuid);

                String transactionUuidSigned = Utilities.parseNameValuePairs(transferDataDecrypted,
                ...
                if (verifyTransactionUuidMessage
                        .equals(PropertyManager.getInstance().getProperty(Constants.TRANSACTION_UUID_VERIFIED))) {

                    String verifySignatureMessage = CryptoUtilities.verifySignatureByUserUuid(userUuid, transactionUuid,
                            transactionUuidSigned);
                    logger.debug("verifySignatureMessage: " + verifySignatureMessage);

                    if (verifySignatureMessage.trim()
                            .equals(PropertyManager.getInstance().getProperty(Constants.SIGNATURE_VERIFIED))) {

                        String sessionUuid = Utilities.parseNameValuePairs(transferDataDecrypted,
                                Constants.SESSION_UUID);
                        logger.debug("sessionUuid: " + sessionUuid);

                        String credentialUuid = Utilities.parseNameValuePairs(transferDataDecrypted,
                                Constants.CREDENTIAL_UUID);
                        ...
                        if (authentication != null) {
                            ...
                            visitor.setRemoteAddress(authentication.getRemoteAddress());
                            visitor.setRemoteHost(authentication.getRemoteHost());
                            visitor.setRefCode(Constants.VISITOR_THREAT_AUTHENTICATION);

                            VisitorThreatDao.create(visitor);

                            AuthenticationDao.cancelAuthentication(userUuid, sessionUuid);

                            responseString = PropertyManager.getInstance().getProperty(Constants.SIGN_ON_SUCCESSFUL);

                        } else {
                            responseString = PropertyManager.getInstance()
                                    .getProperty(Constants.PROBLEM_WITH_AUTHENTICATION_SERVER);
                        }
        ...
        try {
            String encryptedResponseString = CryptoUtilities.generatedEncryptedResponse(transferKeyDecryptedHex,
                    responseString);
